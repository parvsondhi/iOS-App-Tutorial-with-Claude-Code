...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Configure the iOS status bar in src/main.tsx. Import Capacitor and the StatusBar plugin from @capacitor/status-bar. After the imports, add a native platform check that sets the status bar style to Style.Light (dark text for our light theme). Note: setBackgroundColor is Android-only so don't use it — on iOS, the status bar background comes from the app's root view background color."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Audit and fix touch target sizes across our app to meet Apple's 44x44 minimum. Add a .touch-target utility class to src/index.css with min-height: 44px and min-width: 44px — apply it selectively to standalone interactive elements. Also review icon-only buttons — ensure they use at least p-2.5 padding with w-6 h-6 icons to reach the 44px minimum."
