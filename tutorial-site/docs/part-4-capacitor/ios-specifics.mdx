...
Update the fixed elements — header and bottom nav — to respect safe areas:
...
Configure the status bar in your app's entry point:
...
When a text input is focused on iOS, the keyboard slides up. In a web view, this can push the entire page or cover the input. We need to handle this:
...
Apple's Human Interface Guidelines require minimum touch targets of **44×44 points**. Some of our buttons (especially icon-only ones) might be too small. Audit and fix:
...
Before moving to TestFlight, let's make a production build. Remove the dev server config:
