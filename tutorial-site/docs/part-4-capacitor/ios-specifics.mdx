...
if (Capacitor.isNativePlatform()) {
  ...
  // Note: setBackgroundColor is Android-only. On iOS, the status bar
  // background is controlled by the app's root view background color,
  // which we set via CSS (bg-amber-50 on the root element).
}
...
A simple fix: add a utility class for touch targets and apply it selectively:

```css title="src/index.css (add touch target utility)"
/* Utility class for ensuring iOS minimum touch target */
.touch-target {
  min-height: 44px;
  min-width: 44px;
}
