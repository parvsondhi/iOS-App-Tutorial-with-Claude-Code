...
Send this prompt to Claude Code to configure Capacitor, then compare the result with the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Configure capacitor.config.ts for our GratitudeJar app. Set the app ID, name, and web directory. Add a development server block that points to our Vite dev server so we get hot-reload in the Simulator."
  context="The config file tells Capacitor how to build and run the iOS app. The server block enables hot-reload during development."
  expectedOutput="Claude Code will update capacitor.config.ts with the app ID, name, web directory, and development server settings."
  tip="The server block enables live reload during development — you'll remove it before shipping to TestFlight."
/>

Here's the reference for what your config should look like:
...
Everything you've built across Parts 1-3 is about to come to life as a real iOS app. Let's see it happen.
...
### This is it --- your app is running on iOS!

Take a moment to appreciate what just happened. You started with an idea, built a full web app, and now it's running as a native iOS app in the Simulator. This is the same process professional developers use to build the apps on your phone.

**Take a screenshot!** Share it on social media. You earned this.
...
For an even smoother dev experience, send this prompt to Claude Code to add convenience scripts, then compare with the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 7"
  prompt="Add convenience scripts to package.json for our Capacitor iOS development workflow — one for syncing, one for opening Xcode, and one that does both in sequence."
  context="Convenience scripts save you from remembering the full Capacitor CLI commands."
  expectedOutput="Claude Code will add cap:sync, cap:open, and ios:dev scripts to package.json."
  tip="These scripts simplify the Capacitor dev loop so you don't have to remember the full commands."
/>

Here's the reference for what your scripts section should look like:
...
:::info Want to go further? Get it on real iPhones with TestFlight
Part 5 covers getting your app on TestFlight so you (and friends) can install it on real iPhones. This requires an Apple Developer account ($99/year). If you're not ready for that, that's totally fine — running in the Simulator is a major accomplishment and you can always come back to TestFlight later.
:::
