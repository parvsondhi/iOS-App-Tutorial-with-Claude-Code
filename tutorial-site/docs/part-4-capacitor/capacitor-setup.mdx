...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Update capacitor.config.ts with our app settings. Set appId to 'com.yourname.gratitudejar', appName to 'GratitudeJar', webDir to 'dist'. Add a server block with url 'http://localhost:5173' and cleartext true for development hot-reload."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 7"
  prompt="Add convenience scripts to package.json for the Capacitor workflow: 'cap:sync' runs 'cap sync', 'cap:open' runs 'cap open ios', and 'ios:dev' runs both in sequence."
