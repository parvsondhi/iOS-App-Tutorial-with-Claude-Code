...
<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Update src/pages/Create.tsx to use the native camera service instead of a file input. Replace the photo picker section with two buttons: 'Take Photo' (calls takePhoto, shows Camera icon) and 'Library' (calls pickFromLibrary, shows Image icon). Add a photoBlob state for the upload. Also add an uploadPhotoBlob function to src/lib/storage.ts that uploads a Blob directly (without compression, since the camera plugin already compresses). Add hapticLight before camera actions and hapticSuccess after saving."
