...
<ClaudeCodePrompt
  ...
  tip="Claude Code can edit the Info.plist file automatically, saving you from manually editing XML."
/>
...
You'll also need to add a `photoBlob` state and update the upload to use the blob directly. First, add an `uploadPhotoBlob` function to `storage.ts` that uploads a pre-compressed blob (since the Capacitor camera plugin already compresses at 80% quality and 1200×1200):

```ts title="src/lib/storage.ts (add this function)"
export async function uploadPhotoBlob(
  blob: Blob,
  onProgress?: (progress: number) => void
): Promise<string> {
  // No compression needed — the Capacitor camera plugin already handles it
...
Then update Create.tsx:

```tsx title="src/pages/Create.tsx (state additions)"
import { uploadPhoto } from '../lib/storage'
import { uploadPhotoBlob } from '../lib/storage'
