...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="lets add a modal pop animation, so that if a new streak # has been achieved or has incremented, then one i submit the entry and it saves, there is a pop that shows the fire icon flaming and text animation below that shows the previous streak number incrementing to the new value."
  ...
  expectedOutput="Claude Code will create a pop animation modal that appears after saving an entry when the streak increments. It will show an animated fire icon with a number counting up from the previous streak to the new value."
  tip="If you get any terminal errors during this step, paste the full error output directly into Claude Code. It can read error messages and fix them faster than you can search Stack Overflow."
/>
...
You have built a full-featured web application from scratch. Let's take a step back and look at everything you have created across Part 4:
...
Your web app is feature-complete. It has authentication, data persistence, media uploads, habit tracking, and a polished mobile interface. That is a real product.

In Part 5, we take this exact web app and wrap it in Capacitor to turn it into a native iOS app. Same codebase, native superpowers -- camera access, haptic feedback, shake detection, and the App Store.

<TwitterCallout />

**[Next: Chapter 9 -- Capacitor Setup →](../part-5-capacitor/capacitor-setup)**
