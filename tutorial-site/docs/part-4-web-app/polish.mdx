...
<TroubleshootingAccordion items={[
  {
    ...
    solution: <p>Check that <code>localStorage.setItem('gratitude-tree-onboarded', 'true')</code> is being called in your <code>onComplete</code> handler. Open DevTools, go to Application, then Local Storage, and verify the key exists. If it does not, the callback is not wired up correctly in App.tsx.</p>
  },
  {
