...
<ClaudeCodePrompt
  prompt="Review my GratitudeTree app code for anything that shouldn't be in a production build: console.log statements, TODO comments, test data, placeholder text, debug flags, or hardcoded localhost URLs. Check src/ directory recursively."
...
<ClaudeCodePrompt
  prompt="I'm about to submit my GratitudeTree app to the App Store. What are the most common reasons apps get rejected by Apple's review team? Can you review my app's setup for potential issues? The app uses Firebase Auth (email/password), Firestore, Storage, and Capacitor plugins for camera, haptics, and motion detection."
...
<ClaudeCodePrompt
  prompt="I just published my first iOS app (GratitudeTree — a gratitude journal). I want to keep improving it. What features would make the biggest impact? Consider: dark mode, push notification reminders, data export, social sharing, widgets, Apple Watch companion, or something I haven't thought of."
