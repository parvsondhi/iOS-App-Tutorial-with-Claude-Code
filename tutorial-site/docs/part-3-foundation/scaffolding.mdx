...
## Step 3: Set Up Project Context for Claude Code

Before we start building, let's give Claude Code a design brief that applies to every prompt going forward.

<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a CLAUDE.md file in the project root for our GratitudeJar app. Include these directions for any AI assistant working on this project: the app should follow iOS Human Interface Guidelines, use a premium minimal aesthetic with subtle shadows and rounded corners, the color scheme is warm amber tones, the design should look like a professional designer built it, and the tech stack is React + TypeScript + Vite + Tailwind CSS v4 + Firebase + Capacitor. Also reference the data model in docs/data-model.md."
  context="The CLAUDE.md file tells Claude Code about your project's design philosophy and tech stack so you don't have to repeat it in every prompt."
  expectedOutput="Claude Code will create a CLAUDE.md file that acts as a style guide for the entire project. Every prompt you send from now on will automatically pick up these design directions."
...
## Step 5: Clean Up the Starter Files
...
## Step 7: Create the TypeScript Types
...
<ClaudeCodePrompt
  variant="build"
  step="Step 7"
  prompt="Create a TypeScript types file at src/lib/types.ts for our GratitudeJar app. Define an Entry type with: id (string), type ('text' or 'photo'), content (string), caption (optional string), date (string in YYYY-MM-DD format), and createdAt (Firebase Timestamp). Also create a NewEntry type that omits id and createdAt since those are generated by Firebase."
...
If you're using Git, this is a good time to initialize your repository and make your first commit:
...
├── .gitignore
├── CLAUDE.md             # Design brief for Claude Code
├── index.html
├── package.json
├── tsconfig.json
├── tsconfig.app.json
...
- Your project is committed to Git
...
**[Next: Chapter 4 — UI Shell with Tailwind CSS →](../part-4-web-app/ui-shell)**
