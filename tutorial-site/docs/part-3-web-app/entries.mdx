...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a date utility file at src/lib/dates.ts with two functions: getLocalDateString(date?) that takes an optional Date parameter (defaults to new Date()) and returns the date as 'YYYY-MM-DD' using the local timezone (not UTC, to avoid timezone bugs), and formatDisplayDate() that takes a 'YYYY-MM-DD' string and returns a human-friendly format like 'Mar 15, 2025'."
