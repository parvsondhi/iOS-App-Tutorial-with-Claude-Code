...
<ClaudeCodePrompt
  variant="build"
  step="Step 2"
  prompt="Update src/lib/types.ts to add a LocalEntry type that's like Entry but uses a plain Date instead of a Firebase Timestamp for createdAt. We need this for in-memory state before we connect Firebase. Also add a NewEntry type that's an entry without id and createdAt, since those get generated automatically."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a date utility file at src/lib/dates.ts with two functions: one that returns today's date as a 'YYYY-MM-DD' string using the local timezone (not UTC, to avoid timezone bugs), and one that formats a date string into something human-friendly like 'Mar 15, 2025'. The first function should accept an optional Date parameter so we can reuse it later."
  context="Using toISOString() for dates causes timezone bugs — if it's 11 PM in New York, toISOString() returns tomorrow's date because it converts to UTC. Our helper avoids this by using the local timezone. The optional Date parameter lets us reuse this function in Chapters 9 and 10 for converting arbitrary dates."
  expectedOutput="New file src/lib/dates.ts with getLocalDateString(date?: Date) and formatDisplayDate() functions."
/>

### Why not just use `new Date().toISOString().slice(0, 10)`?

Timezones. If it's 11 PM in New York, `toISOString()` returns the next day's date because it converts to UTC. The `getLocalDateString` function Claude Code generated uses the user's local time, which is what they expect when they log a journal entry. It accepts an optional `Date` parameter (defaulting to `new Date()`) so we can reuse it later for converting any date — not just today.
...
<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Replace the placeholder Create page with a real entry creation form. Users should be able to toggle between text and photo entries. For text, show a textarea. For photos, show a file picker with image preview and an optional caption field. Include a Save button that creates the entry and navigates back to the feed. The design should feel polished and premium — like a well-designed iOS app."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Create an EntryCard component at src/components/EntryCard.tsx that displays a single journal entry. It should show the date, the entry content (text or image with caption), and have edit and delete buttons. Clicking edit should let the user change the text right on the card. Clicking delete should show a confirmation before removing. The cards should look polished and premium — with rounded corners, subtle borders, and a clean layout like a well-designed iOS app."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 6"
  prompt="Update the Feed page to show a list of entries using the EntryCard component. It should receive entries and handler functions as props. When there are no entries, show a helpful empty state message. When there are entries, show them in a scrollable list."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 7"
  prompt="Update App.tsx to manage entries state. Add functions to add, edit, and delete entries. Pass entries and these functions as props to Feed and Create. When an entry is saved, automatically switch back to the Feed tab."
...
<ClaudeCodePrompt
  prompt="Can you explain why I shouldn't use push or splice to update my entries array in React state? I want to understand the immutability concept."
