...
We'll create a dedicated file for all Firestore operations. This keeps Firebase logic out of our React components:
...
Custom hooks let us encapsulate complex logic and reuse it across components. Our `useEntries` hook will manage the Firestore subscription:
...
Now we replace the local state management with our Firestore-backed hook:
...
The Feed and Create pages need small updates since we're now using the Firebase `Entry` type instead of `LocalEntry`.

Update the Feed page to handle loading and error states:
...
For web apps, we need to explicitly enable it. Update your Firebase config:
