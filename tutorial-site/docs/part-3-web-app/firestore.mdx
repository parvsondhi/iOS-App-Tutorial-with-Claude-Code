...
> Right now your entries disappear when you refresh. Let's save them to the cloud so they stick around.

This is the chapter where your app becomes real. Instead of storing entries in memory, we'll save them in Firestore — Firebase's cloud database. The idea is simple: when you create an entry, it gets saved to a database in the cloud. When you open the app again, your entries are still there. And the best part — if something changes in the database, the app updates automatically without you having to refresh.
...
<ClaudeCodePrompt
  variant="build"
  step="Step 2"
  prompt="Create a Firestore service that handles saving, updating, deleting, and loading entries from our database. Entries should load in real-time — when something changes in the database, the app should update automatically without refreshing. Save it to src/lib/firestore.ts."
...
Here's a reference implementation. Your code may look different — that's fine as long as it works. **To verify, test it:** we'll test the full flow after wiring everything together in Step 4.
...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a custom React hook at src/hooks/useEntries.ts that connects to our Firestore service. It should track entries, loading state, and errors. When the component loads, start listening for entry changes. When entries are added, edited, or deleted, the real-time listener should handle updating the list automatically. Expose functions to add, edit, and remove entries."
...
Think of it like a radio: when the app opens, it tunes in to your Firestore collection. Any time something changes (an entry is added, edited, or deleted), the radio broadcasts the update and the app displays the new data. When the app closes, it turns the radio off.
...
<ClaudeCodePrompt
  prompt="Can you explain the cleanup pattern in my useEntries hook? Why do I need to unsubscribe when the component unmounts, and what happens if I forget?"
...
Now we replace the local state management with our Firestore-backed hook. Send this to Claude Code, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Update App.tsx to use our new useEntries() hook instead of the local state we had before. Remove the old state management code and pass the hook's entries, loading, error, and handler functions as props to Feed and Create."
...
The Feed and Create pages need small updates since we're now using the Firebase `Entry` type instead of `LocalEntry`. Send this to Claude Code, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Update Feed.tsx to handle loading, error, and empty states. Show a spinner while loading, an error message if something goes wrong, and a helpful empty state when there are no entries. Also update Create.tsx and EntryCard.tsx to use the Firebase Entry type instead of LocalEntry — Firestore handles generating the id and timestamp now, so the Create page doesn't need to provide them."
...
Here's a reference implementation for Feed. Your code may look different — that's fine as long as it works.
...
For web apps, we need to explicitly enable it. Send this to Claude Code, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 6"
  prompt="Update src/lib/firebase.ts to enable offline persistence so the app works even without internet. Use the modern Firebase v10+ approach with initializeFirestore instead of getFirestore."
  context="Offline persistence lets the app work without internet — reads come from IndexedDB cache and writes queue until reconnection. Firebase v10+ enables this through initializeFirestore configuration rather than the deprecated enableIndexedDbPersistence API."
  expectedOutput="Updated firebase.ts using initializeFirestore with persistentLocalCache and persistentMultipleTabManager settings."
  tip="If Claude Code uses the older enableIndexedDbPersistence API, ask it to use initializeFirestore instead — that API was deprecated in Firebase v10."
/>

Here's a reference implementation. Your code may look different — that's fine as long as it works. **To verify:** run the app and check that entries persist after refreshing the page.
...
The `persistentMultipleTabManager()` option enables persistence across multiple browser tabs simultaneously — each tab shares the same cache.

:::info Why not `enableIndexedDbPersistence`?
Older tutorials use `enableIndexedDbPersistence()`, but that API was **deprecated in Firebase v10** (2023). The `initializeFirestore` approach shown above is the modern replacement. It's simpler (no error handling needed) and supports multi-tab by default.
:::
...
Run the app and check that entries persist after refreshing the page. Create an entry, refresh, and it should still be there. That's the big moment!

<ClaudeCodePrompt
  prompt="Review my project and make sure everything is working correctly. Check for any errors or issues with the Firestore integration."
...
<TroubleshootingAccordion items={[
  ...
  },
  {
    ...
    solution: <p>This API was deprecated in Firebase SDK v10+. Use <code>initializeFirestore</code> with <code>persistentLocalCache</code> instead of <code>getFirestore</code> + <code>enableIndexedDbPersistence</code>. See the reference code in Step 6 above.</p>
  }
]} />
