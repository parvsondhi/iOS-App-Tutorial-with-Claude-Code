...
Send this prompt to Claude Code to generate the date utilities, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 2"
  prompt="Add two helper functions to our dates utility: one that generates every day between two dates as an array of date strings, and one that returns the date N weeks ago aligned to start on Monday. We need these to build the heatmap grid."
...
Send this prompt to Claude Code to generate the streak calculator, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a streak calculator at src/lib/streaks.ts. It should take an array of dates and figure out the current streak (consecutive days of journaling), the longest streak ever, and total days active. If the user posted yesterday but not today, the streak should still count — they might journal later today."
...
Send this prompt to Claude Code to generate the heatmap component, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Create a GitHub-style calendar heatmap component showing the last 12 weeks of journaling activity. Each day should be a small colored square — lighter for no entries, darker for more entries. Highlight today's square. Include day labels on the left, month labels on top, and a Less/More legend at the bottom. Use our amber color scheme."
...
<ClaudeCodePrompt
  prompt="Review my project and check for any timezone bugs in the heatmap and streak calculations. Make sure dates aren't off by one day."
...
Send this prompt to Claude Code to update the Profile page with stats and the heatmap, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Update the Profile page to show journaling stats and the calendar heatmap. Show the current streak, best streak, and total days active as stat cards. Add the heatmap below the stats. Update App.tsx to pass the full entries array to Profile instead of just the count."
