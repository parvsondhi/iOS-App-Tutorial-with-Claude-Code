...
Send this prompt to Claude Code to generate the date utilities, then compare with the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 2"
  prompt="Add two new functions to src/lib/dates.ts: getDaysInRange(startDate, endDate) that returns an array of 'YYYY-MM-DD' strings for every day in the range, and getWeeksAgo(weeks) that returns a Date object for N weeks ago adjusted to start on Monday."
  context="These date utilities power the heatmap grid — we need to generate every day in a range and align weeks to start on Monday."
  expectedOutput="Claude Code will add getDaysInRange and getWeeksAgo functions to dates.ts."
/>
...
Send this prompt to Claude Code to generate the streak calculator, then compare with the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a streak calculator at src/lib/streaks.ts. The calculateStreaks function takes an array of date strings ('YYYY-MM-DD') and returns currentStreak, longestStreak, and totalDays. Current streak counts consecutive days backward from today/yesterday. Use Set for deduplication. Handle edge cases: multiple entries per day count as one streak day, and if the user hasn't posted today but posted yesterday, the streak is still alive."
  context="Streak calculation needs careful date math. The key insight: yesterday counts as 'active' for the current streak because the user might journal later today."
  expectedOutput="Claude Code will create src/lib/streaks.ts with a calculateStreaks function that handles deduplication, consecutive-day counting, and both current and longest streak tracking."
/>
...
Send this prompt to Claude Code to generate the heatmap component, then compare with the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Create a GitHub-style calendar heatmap at src/components/CalendarHeatmap.tsx showing the last 12 weeks. Accept entryDates (array of 'YYYY-MM-DD' strings). Use useMemo to: count entries per day with a Map, generate a 7-row grid (Mon-Sun) × N columns (weeks), and extract month labels. Color cells with amber shades based on count (0=amber-100, 1=amber-300, 2=amber-500, 3+=amber-700). Highlight today with a ring. Add day labels (Mon, Wed, Fri) and a Less/More legend."
  context="This is the most complex visual component in the app — a grid of colored cells representing journaling activity over time."
  expectedOutput="Claude Code will create CalendarHeatmap.tsx with useMemo-based grid generation, color-coded cells, month labels, day labels, and a Less/More legend."
/>
...
Send this prompt to Claude Code to update the Profile page with stats and the heatmap, then compare with the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Update src/pages/Profile.tsx to show stats and the heatmap. Change props to accept entries (Entry[]) instead of entryCount. Use useMemo to extract entryDates and calculate streaks. Show three stat cards in a grid: Current Streak (Flame icon), Best Streak (Trophy icon), Days Active (Calendar icon). Add the CalendarHeatmap component in a card. Update App.tsx to pass entries instead of entries.length to Profile."
  context="The Profile page becomes the hub for habit tracking — stats, heatmap, and total entries all in one view."
  expectedOutput="Claude Code will rewrite Profile.tsx with streak stats, the CalendarHeatmap component, and update App.tsx to pass the full entries array."
/>
