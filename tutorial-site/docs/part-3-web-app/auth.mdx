...
Send this prompt to Claude Code to generate the auth hook, then check the reference below:
...
Send this prompt to Claude Code to generate the auth screen, then check the reference below:
...
Send this prompt to Claude Code to update the Profile page, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Update the Profile page to show the logged-in user's info. Display their email, how many entries they have, and a logout button. Replace the placeholder content with real user data."
...
Send this prompt to Claude Code to make entries user-specific, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Make entries private to each user. Each entry should be tagged with the user's ID, and users should only see their own entries. Update the types, Firestore service, and useEntries hook to filter entries by the logged-in user."
...
When you combine `where()` and `orderBy()` on different fields (like `userId` and `createdAt`), Firestore requires a composite index. If you see an error in the console with a link to create the index, click the link — Firebase will create it automatically. This is a one-time setup that takes about a minute.
:::
...
Send this prompt to Claude Code to integrate authentication into the app, then check the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 6"
  prompt="Update App.tsx to require login before using the app. Show a loading spinner while checking auth, the login screen if not logged in, and the full app if logged in. Pass the user info to the Profile page so it can show the email and a logout button."
...
<ClaudeCodePrompt
  prompt="Review my project and make sure the authentication is working correctly. Check for any errors or issues."
