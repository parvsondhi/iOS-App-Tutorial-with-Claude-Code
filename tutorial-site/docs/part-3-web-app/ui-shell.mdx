...
<ClaudeCodePrompt
  variant="build"
  step="Step 2"
  prompt="Create a header for the app with the GratitudeJar name and a flower icon. It should stay fixed at the top of the screen and use our warm amber color scheme. Save it to src/components/Header.tsx."
  ...
  tip="Read through the Tailwind classes Claude Code used — understanding them now will help you style everything else in the tutorial."
/>

Here's a reference implementation. Your code may look different — that's fine as long as it works. **To verify, test it:** run the dev server and check that a header appears fixed at the top of the screen with the app name and flower icon.
...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a bottom navigation bar with three tabs: Feed, New (with a plus icon), and Profile. It should stay fixed at the bottom of the screen. The active tab should be highlighted. It needs to accept the current active tab and a callback to change tabs as props. Save it to src/components/BottomNav.tsx."
  ...
  tip="Notice how Claude Code uses TypeScript's union type for Tab — this means if you accidentally misspell a tab name, TypeScript catches it at compile time."
/>

Here's a reference implementation. Your code may look different — that's fine as long as it works. **To verify, test it:** check that a nav bar appears at the bottom with three tabs and that tapping each one highlights it.
...
We need three placeholder pages for each tab. These will be simple for now — Claude Code will fill them with real content in later chapters.

<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Create three placeholder page components: Feed, Create, and Profile. Each should show a centered icon, a title, and a short description of what will be built there later. Use our amber color scheme. Save them to src/pages/Feed.tsx, src/pages/Create.tsx, and src/pages/Profile.tsx."
  ...
  tip="Placeholder screens are a professional practice — they let you build navigation and layout before the real content exists."
/>
...
<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Wire together the app shell in App.tsx. Import all the components we just created. Add state to track which tab is active (default to the feed). Show the header at the top, the active page in the middle, and the bottom nav at the bottom. Make sure the content area fills the space between the header and nav."
...
Here's a reference implementation. Your code may look different — that's fine as long as it works. **To verify, test it:** run `npm run dev`, open the app, and confirm you can switch between tabs.
...
<ClaudeCodePrompt
  prompt="Review my project and make sure everything is working correctly. Check for any errors or issues, especially around mobile layout and fixed positioning."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 6"
  prompt="Update index.html so the app renders correctly on iPhones with notches. We need viewport-fit=cover on the viewport meta tag."
