...
<ClaudeCodePrompt
  variant="build"
  step="Step 2"
  prompt="Create a skeleton loading card that shows the shape of an entry card while data is loading. It should have animated placeholder bars that pulse. Then update Feed.tsx to show three of these skeleton cards instead of a spinner during loading."
...
Here's a reference implementation. Your code may look different — that's fine as long as it works.
...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a nice empty state component for when there are no entries. Show an encouraging message like 'Your jar is empty' with a button to create the first entry. Update Feed.tsx to show this instead of a blank screen, and wire up the button to navigate to the Create tab."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Create a welcome onboarding screen that first-time users see. It should have three slides explaining what the app does: welcome, write and snap photos, and shake to discover old entries. Include Next/Get Started buttons, dot indicators, and a Skip option. Only show it once — use localStorage to remember if the user has already seen it."
...
<ClaudeCodePrompt
  prompt="Should I add swipe gestures to my onboarding screen so users can swipe between slides on mobile?"
...
<ClaudeCodePrompt
  variant="build"
  step="Step 6"
  prompt="Add a function to our dates utility that shows relative timestamps like 'Just now', '5m ago', '2h ago', 'Yesterday' — more natural than showing the full date for recent entries. Fall back to the full date for entries older than a week."
...
<ClaudeCodePrompt
  prompt="Review my project and make sure everything is working correctly. Check for any errors or issues."
