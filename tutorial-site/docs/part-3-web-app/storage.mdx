...
Phone cameras take photos at 3000x4000 pixels or more. Uploading those full-size images wastes bandwidth and makes the feed slow. Let Claude Code build the compression utility:

<ClaudeCodePrompt
  variant="build"
  step="Step 2"
  prompt="Create an image compression utility at src/lib/image.ts. It should take a photo file, shrink it down to a reasonable size (no bigger than 1200px), and convert it to JPEG. Phone cameras take huge photos and we don't want to upload 4MB files when 200KB will look just as good."
  context="Compressing images client-side before uploading saves bandwidth, reduces storage costs, and makes the feed load faster."
  expectedOutput="A new file src/lib/image.ts with a compressImage function that takes a File and returns a Promise<Blob>."
  tip="The function uses the Canvas API — if Claude Code's version looks different from the reference, that's fine as long as it resizes and compresses."
/>
...
export async function compressImage(file: File): Promise<Blob> {
  return new Promise((resolve, reject) => {
    const img = new Image()
    img.onload = () => {
      // Free the object URL to prevent memory leaks
...
Let Claude Code build the upload service, then compare with the reference below:

<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a Firebase Storage upload service at src/lib/storage.ts. It should compress the image first using our compression utility, then upload it to Firebase Storage with a progress callback so we can show a progress bar. It should also have a function to delete photos when entries are removed. Return the permanent download URL after upload."
  context="uploadBytesResumable supports progress tracking, unlike the simpler uploadBytes. The filename pattern entries/timestamp-random.jpg keeps Storage organized and avoids collisions."
  expectedOutput="A new file src/lib/storage.ts with uploadPhoto (returns Promise<string> for the download URL) and deletePhoto functions."
  tip="If Claude Code imports compressImage from ./image, that's correct — the two services work together."
/>
...
export async function deletePhoto(url: string): Promise<void> {
  try {
    // Firebase download URLs contain the storage path encoded in the URL.
    // Extract the path by parsing the '/o/' segment and decoding it.
    const path = decodeURIComponent(url.split('/o/')[1].split('?')[0])
    const storageRef = ref(storage, path)
    await deleteObject(storageRef)
  } catch (err) {
    // If the file doesn't exist or URL parsing fails, that's fine
...
Replace the file input's local preview logic with actual uploads. Let Claude Code build it:

<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Update the Create page to use real Firebase Storage uploads instead of local previews. When a user saves a photo entry, compress and upload the image, show a progress bar during upload, and disable the form so they can't accidentally submit twice. Keep the local preview so users can see what they're about to upload."
  context="This connects Create.tsx to Firebase Storage — photo entries go from temporary local URLs to permanent cloud-hosted images."
  expectedOutput="An updated Create.tsx that imports uploadPhoto from ../lib/storage, tracks upload progress, shows a progress bar, and disables the form during upload."
  tip="If Claude Code keeps the old URL.createObjectURL logic for the preview, that's correct — the preview is local, but the final content URL comes from Firebase Storage."
/>

Here's a reference implementation. Your code may look different — that's fine as long as it works. **To verify, test it:** create a photo entry, check that the progress bar appears, refresh the page, and confirm the photo is still there.
...
<ClaudeCodePrompt
  prompt="My upload progress bar jumps from 0% to 100% instantly for small files. Is there a way to make it smoother?"
...
When a user deletes a photo entry, we should also delete the image from Storage. Let Claude Code update both files:

<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="When a user deletes a photo entry, we should also delete the image file from Storage so we don't waste space. Update the deleteEntry function in firestore.ts to clean up photos, and update the useEntries hook to pass the entry data along when deleting."
  context="Without this cleanup, deleted photo entries leave orphaned files in Storage — wasting space and money over time."
  expectedOutput="Updated firestore.ts with deleteEntry accepting an optional entry parameter and calling deletePhoto for photo entries, plus updated useEntries.ts removeEntry that finds the entry before deleting."
  tip="If Claude Code puts the deletePhoto call before deleteDoc, suggest moving it after — delete the database record first, then clean up the file."
/>
...
service firebase.storage {
  match /b/{bucket}/o {
    match /entries/{filename} {
      ...
      // Allow writes with a 10MB file size limit
      // (we'll restrict to authenticated users in Chapter 8)
...
Create a photo entry, check that the progress bar shows during upload, refresh the page, and confirm the photo is still displayed. Then delete the photo entry and check Firebase Console Storage to confirm the file was removed.

<ClaudeCodePrompt
  prompt="Review my project and make sure everything is working correctly. Check for any errors or issues with the Storage integration."
