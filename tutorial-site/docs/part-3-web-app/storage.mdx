...
<ClaudeCodePrompt
  variant="build"
  step="Step 2"
  prompt="Create an image compression utility at src/lib/image.ts. The compressImage function should take a File, load it into an Image element, resize it proportionally to max 1200x1200, draw it to a Canvas, and export as JPEG at 80% quality. This reduces 4MB phone photos to ~200-400KB."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 3"
  prompt="Create a Firebase Storage upload service at src/lib/storage.ts with two functions: uploadPhoto (compresses the image first, uploads using uploadBytesResumable with progress tracking via state_changed event, returns the permanent download URL) and deletePhoto (deletes a file from Storage by URL, with try-catch since missing files are fine). Use a filename pattern of entries/timestamp-random.jpg."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Update src/pages/Create.tsx to use real Firebase Storage uploads instead of local previews. Add photoFile, uploading, and uploadProgress state. The handleSubmit should: for photo entries, call uploadPhoto with the file and progress callback, then create the Firestore entry with the download URL as content. Show a progress bar during upload and disable the form."
...
<ClaudeCodePrompt
  variant="build"
  step="Step 5"
  prompt="Update src/lib/firestore.ts deleteEntry function to also clean up photos from Storage. Accept an optional entry parameter, and if it's a photo entry, call deletePhoto with the content URL. Update the useEntries hook's removeEntry to find the entry and pass it to deleteEntry."
