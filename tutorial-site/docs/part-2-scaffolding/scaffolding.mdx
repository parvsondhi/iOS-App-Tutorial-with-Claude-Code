...
description: Create the GratitudeTree project with Vite, React, TypeScript, install all dependencies, and set up Firebase.
---
...
<ClaudeCodePrompt
  variant="build"
  step="Step 4"
  prompt="Clean up the Vite starter files for our GratitudeTree app. Replace src/App.tsx with a minimal component that shows 'GratitudeTree' centered on the screen with a warm amber background using Tailwind. Delete the unused react.svg and vite.svg files. Update the page title in index.html to 'GratitudeTree'."
...
![First load in the browser after `npm run dev`](/img/default-first-load-screen.png)
...
![Firebase console — creating or viewing a project](/img/firebase-first-page.png)
...
![Firebase console — Build menu (Firestore, Storage, Auth, etc.)](/img/firebase-console-menu.png)
